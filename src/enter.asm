...
start:                  
    ...
    mov     AH, 09h
    mov     DX, offset prompt_string	;где offset определяет адрес переменной
    int     21h                         ;печатаем строку prompt_string
...
read_file_name proc near                ;пытается прочитать имя файла из префикса сегмента программы, если неудачно, читает его из консоли
	...
	read_from_console:     
		...
		mov     AH, 09h
		int     21h                         ;напечатать prompt_string для пользователя, чтобы он ввел имя файла
...
prompt_string           db      'Type a text (has to less than 4096 symbols):', 0Dh, 0Ah, 24h
